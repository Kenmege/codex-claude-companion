# codex-plugin-cc v1.0.3 — first public OSS release

First version published to GitHub Packages with `--access public` and
provenance attestation. Earlier 1.0.0–1.0.2 tags were private-era launch
attempts and should not be depended on.

`codex-plugin-cc` v1.0.3 turns Claude into a read-only adversarial reviewer for
...
- **Release and workflow hardening.** The repo now has a Node 18/20/22 CI
  ...
  release gating, tag/package version matching, public package publishing with
  provenance, and reviewer dogfooding via Copilot plus Claude Code Action
  wiring.
...
This is the first public OSS v1 release. There is no migration step for new
users:
...
local Codex marketplace so the command surface and helper binary match v1.0.3.

## Security Hardening

The v0.2.x to v1.0.3 hardening pass focused on control-plane correctness:
...
- GitHub Packages install from developer machines still requires npm auth for
  `npm.pkg.github.com`.
- Windows is not a first-class tested platform for v1.0.3.
